...
#'
#' Given a VA-Calibration fit using \link{vacalibration}, this function plots misclassification matrix used in VA-Calibration, and compares uncalibrated and calibrated estimates of cause-specific mortality fractions (CSMFs).
#'
#' @param vacalib_fit Fitted object from \code{vacalibration()}
#'
#' @param toplot Character. What to plot.
#'
#' When \code{toplot="missmat"} and \code{missmat_type="fixed"}, it plots the fixed misclassification matrix used in calibration. When \code{missmat_type} equals \code{"fixed"} or \code{"samples"}, it plots the average misclassification matrix.
#'
#' When \code{toplot="csmf"}, it compares uncalibrated and calibrated estimates of cause-specific mortality fractions (CSMFs).
#'
#' When \code{"both"}, it plots both the misclassification matrix and estimates of CSMFs.
#'
#' @return It returns a plot comparing misclassification matrix used in calibration, and uncalibrated and calibrated estimates of cause-specific mortality fractions (CSMFs).
#'
#' @examples
#'
#' \donttest{
...
#' }
#'
#' @aliases plot_vacalib
#'
#' @import patchwork
#' @import ggplot2
#'
#' @export
plot_vacalib <- function(vacalib_fit, toplot = "both"){

  if(vacalib_fit$input$missmat_type=="fixed"){

    plot_vacalib_fixed(vacalib_fit = vacalib_fit, toplot = toplot)

  }else if(vacalib_fit$input$missmat_type %in% c("prior", "samples")){

    plot_vacalib_prior(vacalib_fit = vacalib_fit, toplot = toplot)
